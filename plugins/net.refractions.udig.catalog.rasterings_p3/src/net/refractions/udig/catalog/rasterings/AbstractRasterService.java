...
public abstract class AbstractRasterService extends IService {
     ...
     */
    public AbstractGridCoverage2DReader getReader(IProgressMonitor monitor) {
        return getReader(null, monitor);
    }
    
    public synchronized AbstractGridCoverage2DReader getReader(CoordinateReferenceSystem crs, IProgressMonitor monitor) {
...
//	                }
	                File file = id.toFile();
	                if( file != null ){
	                	// _p3: uncomment the following lines so we can set CRS on the ILayer
	                    // and hint the reader here
	                    if (crs != null) {
...
// XXX _p3: overwrite final method
//    @Override
