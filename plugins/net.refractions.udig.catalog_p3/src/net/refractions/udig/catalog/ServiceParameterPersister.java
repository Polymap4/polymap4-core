...
public class ServiceParameterPersister {
	...
	public void restore(Preferences node) {
		try {
			for (String id : node.childrenNames()) {
			    System.out.println( "ServiceParameterPersister: id= " + id );
				try {
				    Preferences servicePref = node.node(id);
					ID url = toId(id);
	                System.out.println( "ServiceParameterPersister: url= " + url );
	...
	protected void locateService(ID targetID, Map<String, Serializable> connectionParameters,  Map<String,Serializable> properties) {
        ...
        System.out.println( "    newServices= " + newServices );
		if( !newServices.isEmpty() ){
			for (IService service : newServices) {
			    ...
			    if (found == null && serviceID.equals(targetID)) {
			        ...
			        try {
			            ...
			            System.out.println( "    error while restoring properties: " + e );
			            e.printStackTrace();
			        }
			    }
			    else {
			        // Service was already available
		            System.out.println( "    already found: " + serviceID );
			    }
		    }
		} else {
    ...
    public void store( IProgressMonitor monitor, Preferences node,
            Collection<? extends IResolve> resolves )
...
//
//                        String path = serviceID.toFile().getAbsolutePath();
...
//                        id = URLEncoder.encode( path, ENCODING );
                    ...
                    if (serviceID.getTypeQualifier() != null) {
                        id = id + TYPE_QUALIFIER
                                + URLEncoder.encode( serviceID.getTypeQualifier(), ENCODING );
                    }
                }
                ...
                for (Map.Entry<String, Serializable> entry : service.getConnectionParams()
                    ...
                    String value;
                    // if reference is null then we can only encode the absolute
                    // path
