...
public abstract class IService 
        implements IResolve, IAdaptable {

    private static Log log = LogFactory.getLog( IService.class );
    ...
    /**
     * This is a protected field; that is lazily created when getInfo is called.
     */
	protected volatile IServiceInfo info = null;

	
	/**
	 * _p3: Make the resolve mechanism available via the {@link IAdaptable}
	 * interface. This helps to add eclipse/workbench related APIs to service,
	 * such as the ACL API.
	 * <p>
	 * I recently found {@link IResolveAdapterFactory}; but this does not seem
	 * to work for all types (?)
	 */
    public Object getAdapter( Class adapter ) {
        // try resolve
        try {
            Object resolved = resolve( adapter, null );
            if (resolved != null) {
                log.info( "getAdapter(): adapter= " + adapter.getSimpleName() + ", resolved= " + resolved );
                return resolved;
            }
        }
        catch (IOException e) {
            log.warn( "getAdapter(): ...", e );
        }
        ...
        log.info( "getAdapter(): adapter= " + adapter.getSimpleName() + ", result= " + result );
        return result;
    }
