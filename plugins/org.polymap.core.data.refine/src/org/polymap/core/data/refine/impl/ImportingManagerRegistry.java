...
public class ImportingManagerRegistry {
    ...
    public static void registerFormats() {
        /*
         * Formats and their UI class names and parsers: - UI class names are used on
         * the client-side in Javascript to instantiate code that lets the user
         * configure the parser's options. - Parsers are server-side code that do the
         * actual parsing. Because they have access to the raw files, they also
         * generate defaults for the client-side UIs to initialize.
         */
         ...
         * mime-types, look at the actual files' content, and try to guess a better
         * format.
         */
    ...
    public static void registerControllers() {
        /*
         * Controllers: these implement high-level UI flows for importing data. For
         * example, the default controller lets the user specify one or more source
         * files, either local or remote or on the clipboard, lets the user select
         * which files to actually import in case any of the original file is an
         * archive containing several files, and then lets the user configure parsing
         * options.
         */
