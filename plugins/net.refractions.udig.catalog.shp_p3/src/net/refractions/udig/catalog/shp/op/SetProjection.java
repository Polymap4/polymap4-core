...
public class SetProjection implements IOp {
	...
	public void op( final Display display, final Object target,
        ...
        for (int i = 0; i < allDatastores.length; i++) {
            ...
            allDatastores[i] = AdapterUtil.instance.adaptTo( ShapefileDataStore.class, object, monitor );
            // _p3: allow to use this operation the shp geo resource too
            if (allDatastores[i] == null) {
                ShpGeoResourceImpl geores = AdapterUtil.instance.adaptTo( ShpGeoResourceImpl.class, object, monitor );
                if (geores != null) {
                    allDatastores[i] = geores.service().resolve( ShapefileDataStore.class, monitor );
                }
            }
            
            if (crs == null) {
                crs = AdapterUtil.instance.adaptTo( CoordinateReferenceSystem.class, object, monitor );
            }
        }
