...
class FeatureTableContentProvider implements ILazyContentProvider, IProvider<Collection<SimpleFeature>> {
    ...
    private CollectionListener listener = new CollectionListener(){
        public void collectionChanged( CollectionEvent event ) {
            ...
            final TableViewer viewer = FeatureTableContentProvider.this.owningFeatureTableControl
                    .getViewer();
            ...
            // _p3: reload content for every event type
            final Table table = viewer.getTable();
            table.getDisplay().asyncExec(new Runnable(){
                public void run() {
                    // FIXME in case the event has no features we need to
                    // reload everything to update #features; otherwise the
                    // following line should be enough
                    //viewer.getTable().clearAll();
                    inputChanged( viewer, input, input );
                }
            });
        }
    };
