...
public class UDIGDropHandler extends DropTargetAdapter {
    ...
    // _p3: static sharing over all sessions is not a good idea in RAP
    ...
    public CompositeDropActionJob getActionJob() {
        // _p3: I do not see the need of a single job queue; in almost all cases there is
        // one drop event at a given time; besides the static job instance would not work
        // with RAP
        // return actionJob;
        
        return new CompositeDropActionJob();
    }
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                synchronized (this) {
                    ...
                    if (next == null || Thread.currentThread().isInterrupted()) {
                        System.out.println( "Returning from CompositeDropActionJob...." );
                        return Status.OK_STATUS;
                    }
                }

                boolean foundGoodAction = false;
                for (Iterator<DropActionRunnable> iterator = next.iterator(); iterator.hasNext()
                        && !foundGoodAction;) {
                    ...
                    if (action.status == Status.OK_STATUS) {
