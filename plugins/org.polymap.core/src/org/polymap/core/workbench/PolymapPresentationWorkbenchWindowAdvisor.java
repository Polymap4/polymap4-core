 ...
 */
public class PolymapPresentationWorkbenchWindowAdvisor
    ...
    public PolymapPresentationWorkbenchWindowAdvisor( final IWorkbenchWindowConfigurer configurer ) {
        super( configurer );
    }
    ...
    private void createCoolBar( final Composite banner, final Control leftControl ) {
        IWorkbenchWindowConfigurer configurer = getWindowConfigurer();
            ...
            actions.add( new Action() {
                ...
                public void run() {
                    ...
                    final PopupDialog popupDialog = new PopupDialog( shell,
                            SWT.RESIZE | SWT.ON_TOP, false, false, false, false, null, null ) {
                ...
                private Rectangle calculatePopUpBounds( final Composite banner,
                        final Composite menuBar, final Composite content ) {
                    Rectangle menuBarBounds = menuBar.getBounds();
                    Rectangle bannerBounds = banner.getBounds();
                    Display display = menuBar.getDisplay();
                    Shell shell = menuBar.getShell();
                    Point menuBarPosition = display.map( menuBar.getParent(), shell, menuBar
                            .getLocation() );
                    Point bannerPosition = display.map( banner.getParent(), shell, banner
                            .getLocation() );

                    return new Rectangle( bannerPosition.x, bannerBounds.height - 5,
                            menuBarBounds.width + 10, content.getSize().y + 10 );
                }
            } );
        }
