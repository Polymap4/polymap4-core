...
final class SpnegoFilterConfig { // NOPMD
     ...
     * @throws URISyntaxException if path to login conf is bad
     */
    static SpnegoFilterConfig getInstance(final ServletConfig config) 
        throws FileNotFoundException, URISyntaxException {
     ...
     * Class is a Singleton. Use the static getInstance() method.
     */
    private SpnegoFilterConfig(final ServletConfig config) 
            throws FileNotFoundException, URISyntaxException {
...
//        assert loginConfExists(config.getInitParameter(Constants.LOGIN_CONF));
        ...
        File workspace = Polymap.getWorkspacePath().toFile();
        ...
        // determine if all req. met to use keyTab
        doServerModule( "spnego-server" );
        
        // FIXME if username/password provided, don't use key tab 
        setUsernamePassword( "admin", "admin" );  //config.getInitParameter( Constants.PREAUTH_USERNAME ), config.getInitParameter( Constants.PREAUTH_PASSWORD ) );
        
        // determine if we should support Basic Authentication
        setBasicSupport( Boolean.FALSE.toString(), Boolean.FALSE.toString() );
        
        // determine if we should Basic Auth prompt if rec. NTLM token
        setNtlmSupport( Boolean.FALSE.toString() );  //config.getInitParameter(Constants.PROMPT_NTLM));
        
        // requests from localhost will not be authenticated against the KDC 
        if (null != config.getInitParameter(Constants.ALLOW_LOCALHOST)) {
            this.allowLocalhost = false;  //Boolean.parseBoolean(config.getInitParameter(Constants.ALLOW_LOCALHOST));
        }
        
        // determine if the server supports credential delegation 
        if (null != config.getInitParameter(Constants.ALLOW_DELEGATION)) {
            this.allowDelegation = true;  //Boolean.parseBoolean(config.getInitParameter(Constants.ALLOW_DELEGATION));
        }
    }
