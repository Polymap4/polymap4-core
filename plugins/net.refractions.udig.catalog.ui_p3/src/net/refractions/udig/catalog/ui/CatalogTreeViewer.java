...
public class CatalogTreeViewer extends TreeViewer implements ISelectionChangedListener {
    ...
    public CatalogTreeViewer( Composite parent, int style, boolean titles ) {
        ...
        catalogs.add( CatalogPlugin.getDefault().getLocalCatalog() ); 
        setInput( catalogs );
        expandToLevel( 2 );
        ...
        // _p3:
        CatalogPlugin.getDefault().getLocalCatalog().addCatalogListener( new IResolveChangeListener() {
            public void changed( IResolveChangeEvent ev ) {
                System.out.println( "Catalog changed. ev=" + ev.toString() );
//                refresh( true );
//                expandAll();
