 ...
 * @since 3.0
 */
public class ZoomFeaturesAction
        implements IViewActionDelegate {

    private static Log log = LogFactory.getLog( ZoomFeaturesAction.class );
    ...
    public void run( IAction action ) {
        if (selectionFilter != null) {
            try {
                ...
                FeatureCollection<SimpleFeatureType, SimpleFeature> features = fs.getFeatures( selectionFilter );
                
                BoundsVisitor bounds = new BoundsVisitor();
                features.accepts( bounds, new NullProgressListener() );
                ReferencedEnvelope bbox = bounds.getBounds();

//                ReferencedEnvelope bbox = features.getBounds();
                log.info( "Feature bbox: " + bbox );
                view.getLayer().getMap().setExtent( bbox );
            }
            catch (Exception e) {
